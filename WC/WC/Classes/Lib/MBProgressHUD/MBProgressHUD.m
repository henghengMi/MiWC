...
#pragma mark  --- 解决传值不到位问题
- (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated
{
    [view addSubview:self];
    [self show:animated];
    return MB_AUTORELEASE(self);
}
...
//#warning 设置弹出的背景和透明度
		self.opacity = 1.0f; // 透明度
        self.color = nil;   // 设置弹出的背景
		...
		self.opaque = NO;
//		self.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.4f];
...
#pragma mark - Internal show & hide operations（出来和隐藏的动画）
- (void)showUsingAnimation:(BOOL)animated {
    // 如果是方法缩小
    ...
    else if ( animationType == MBProgressHUDAnimationZoomRotate)
    {
...
//            self.transform = CGAffineTransformMakeScale(-0.5, -0.5);
    ...
    // Fade in
    else if (animated && animationType == MBProgressHUDAnimationFade) {
...
- (void)hideUsingAnimation:(BOOL)animated {
	...
	if (animated && showStarted) {
        ...
        NSLog(@"hideUsingAnimation:%d",animationType);
...
//    [label.text boundingRectWithSize:CGSizeMake(MAXFLOAT, MAXFLOAT) options:<#(NSStringDrawingOptions)#> attributes:<#(nullable NSDictionary<NSString *,id> *)#> context:<#(nullable NSStringDrawingContext *)#>];
...
- (void)drawRect:(CGRect)rect {
	...
	if (self.dimBackground) {
		...
		//Gradient center
		CGPoint gradCenter= CGPointMake(self.bounds.size.width * 0.5, self.bounds.size.height * 0.5);
		//Gradient radius
